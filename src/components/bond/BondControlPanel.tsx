...
interface BondControlPanelProps {
  viewDef: DetailViewDef;
}

export function BondControlPanel({ viewDef }: BondControlPanelProps) {
  ...
  const guideText = viewDef.id === 'bond-kr'
    ? '채권시장에 영향을 주는 변수를 조절하며 금리와 신용이 채권 가격에 어떻게 반영되는지 살펴보세요.'
    : '미국 채권시장에 영향을 주는 변수를 조절하며 금리와 신용이 채권 가격에 어떻게 반영되는지 살펴보세요.';
