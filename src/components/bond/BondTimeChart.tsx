 ...
 * 채권 시계열 차트 (SVG)
 *
 * 채권 압력이 시간대별로 어떻게 반영되는지 보여준다.
 * x축: 즉시 / 단기 / 중기
 * 단일 라인: 채권 압력
 */

import { useStateColors } from '../../hooks/useStateColors';
import type { BondTimeSeriesPoint } from '../../hooks/useBondSimulation';

interface BondTimeChartProps {
  data: BondTimeSeriesPoint[];
  label: string;
}
...
export function BondTimeChart({ data, label }: BondTimeChartProps) {
