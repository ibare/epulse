 ...
 * 환율 시계열 차트 (SVG)
 *
 * 환율 압력이 시간대별로 어떻게 반영되는지 보여준다.
 * x축: 즉시 / 단기 / 중기
 * 단일 라인: 환율 압력
 */

import { useStateColors } from '../../hooks/useStateColors';
import type { FxTimeSeriesPoint } from '../../hooks/useFxSimulation';

interface FxTimeChartProps {
  data: FxTimeSeriesPoint[];
  label: string;
}
...
export function FxTimeChart({ data, label }: FxTimeChartProps) {
