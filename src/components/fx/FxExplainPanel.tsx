 ...
 * 환율 뷰 우측 설명 패널
 *
 * 환율 상태 요약 + 선택 노드 설명 + 시계열 차트
 */

import { useSimulationStore } from '../../store/simulationStore';
import { useStateColors } from '../../hooks/useStateColors';
import { useFxSimulation } from '../../hooks/useFxSimulation';
...
import { FxTimeChart } from './FxTimeChart';
import type { DetailViewDef } from '../../domain/views/types';

interface FxExplainPanelProps {
  viewDef: DetailViewDef;
  selectedNodeId: string | null;
}

export function FxExplainPanel({ viewDef, selectedNodeId }: FxExplainPanelProps) {
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-4">
      ...
      <div className="rounded-lg px-4 py-3" style={{ backgroundColor: 'rgba(30,41,59,0.4)' }}>
        ...
        {resultState && (
          <div className="mb-3">
            ...
            >
              <div className="mb-1 text-[10px] font-medium text-slate-500">환율 압력</div>
      ...
      {/* 시계열 차트 */}
      <FxTimeChart data={timeSeriesData} label="환율 압력" />
    </div>
  );
}
