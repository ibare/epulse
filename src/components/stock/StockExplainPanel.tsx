...
import { StockTimeChart } from './StockTimeChart';
import type { DetailViewDef } from '../../domain/views/types';

interface StockExplainPanelProps {
  viewDef: DetailViewDef;
  selectedNodeId: string | null;
}

export function StockExplainPanel({ viewDef, selectedNodeId }: StockExplainPanelProps) {
  ...
  const resultLabel = viewDef.id === 'stock-kr' ? '한국 주가' : '미국 주가';
  const chartLabel = viewDef.id === 'stock-kr' ? '한국 주가 압력' : '미국 주가 압력';
  ...
  return (
    <div className="flex h-full flex-col gap-4 overflow-y-auto p-4">
      ...
      {/* 시계열 차트 */}
      <StockTimeChart data={timeSeriesData} label={chartLabel} />
    </div>
  );
}
